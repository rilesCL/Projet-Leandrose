...
export default function OfferCandidaturesList() {
    ...
    const downloadCvHandler = async (candidature) => {
        try {
            const baseName =
                [candidature.studentFirstName, candidature.studentLastName].filter(Boolean).join(' ') || 'etudiant';
    ...
    const handleConvocationSubmit = async (e) => {
        ...
        try {
            ...
            setCandidatures((prev) =>
                prev.map((c) =>
                    c.id === selectedCandidature.id ? { ...c, status: 'CONVENED' } : c
                )
            );
        } catch (error) {
    ...
    const renderStatus = (status) => (
        <span className="text-xs font-medium text-gray-700">
            {t(`employerCandidatures.status.${status}`)}
        </span>
    );
    ...
    if (loading)
        return (
            <div className="bg-white p-6 shadow rounded text-center">
                {t('internshipOffersList.loading')}
            </div>
        );
    if (error)
        return (
            <div className="bg-white p-6 shadow rounded text-center text-red-600">
                <p className="mb-4">
                    {t('internshipOffersList.errorTitle')}: {error}
                </p>
                <button
                    onClick={load}
        ...
        return (
            <div className="bg-white p-6 shadow rounded text-center">
                <h3 className="text-lg font-semibold mb-2">
                    {t('employerCandidatures.emptyTitle')}
                </h3>
                <p className="text-gray-600 mb-4">
                    {t('employerCandidatures.emptyDescription')}
                </p>
                ...
                <div className="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div>
                        <h2 className="text-lg font-semibold">
                            {t('employerCandidatures.titleOffer')} #{offerId}
                        </h2>
                        <p className="text-sm text-gray-500">
                            {t('employerCandidatures.candidaturesCount', {
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {t('employerCandidatures.table.student')}
                            </th>
                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {t('employerCandidatures.table.program')}
                            </th>
                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {t('employerCandidatures.table.date')}
                            </th>
                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {t('employerCandidatures.table.status')}
                            </th>
                            <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                {t('employerCandidatures.table.actions')}
                            </th>
                        </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-100">
                        {candidatures.map((c) => {
                            const date = c.applicationDate
                            ...
                            const displayName =
                                [c.studentFirstName, c.studentLastName]
                                    .filter(Boolean)
                                    .join(' ') || '-';
                            const programDisplay = c.studentProgram
                                ? t(c.studentProgram)
                                : '-';
                                    ...
                                    <td className="px-4 py-2 text-sm space-x-2">
                                        ...
                                        {canAcceptOrReject(c.status) && (
                                            <>
                                                ...
                                                >
                                                    {t('employerCandidatures.actions.accept')}
                                                </button>
                                                <button
                                                    onClick={() =>
                                                        handleReject(c)
                ...
                >
                    ...
                    >
                        ...
                        <h3 className="text-lg font-semibold mb-4 pr-8">
                            {t('employerCandidatures.convocationModal.title')} -{' '}
                            {[selectedCandidature.studentFirstName, selectedCandidature.studentLastName]
                                .filter(Boolean)
                                .join(' ')}
                        </h3>
