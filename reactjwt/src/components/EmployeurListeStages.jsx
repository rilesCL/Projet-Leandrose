...
export default function EmployeurListeStages() {
    ...
    const getStatusBadge = (entente) => {
        ...
        if (entente.statut === 'VALIDEE') {
            return <span className="px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">{t("ententeStage.status.validation")}</span>;
        }

        if (employerSigned && entente.statut === 'EN_ATTENTE_SIGNATURE') {
            return <span className="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{t("ententeStage.status.waiting_other_signatures")}</span>;
        }

        if (entente.statut === 'EN_ATTENTE_SIGNATURE') {
            return <span className="px-2 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800">{t("ententeStage.status.waiting_your_signature")}</span>;
        }

        if (entente.statut === 'BROUILLON') {
            return <span className="px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">{t("ententeStage.status.draft")}</span>;
        }
