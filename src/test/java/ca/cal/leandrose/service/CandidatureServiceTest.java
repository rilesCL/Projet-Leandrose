...
class CandidatureServiceTest {
  ...
  void postuler_ShouldThrow_WhenCvNotApproved() {
    Cv rejectedCv =
        cvRepository.save(Cv.builder().student(testStudent).status(Cv.Status.PENDING).build());

    assertThrows(
        IllegalStateException.class,
        () ->
            candidatureService.postuler(
                testStudent.getId(), testOffer.getId(), rejectedCv.getId()));
  }
  ...
  void acceptByEmployeur_ShouldThrow_WhenAlreadyRejected() {
    ...
    assertThrows(
        IllegalStateException.class, () -> candidatureService.acceptByEmployeur(c.getId()));
  }
  ...
  void acceptByStudent_ShouldThrow_WhenNotOwnedByStudent() {
    ...
    assertThrows(
        IllegalStateException.class, () -> candidatureService.acceptByStudent(c.getId(), 999L));
  }
  ...
  void rejectByStudent_ShouldThrow_WhenNotOwnedByStudent() {
    ...
    assertThrows(
        IllegalStateException.class, () -> candidatureService.rejectByStudent(c.getId(), 999L));
  }
  ...
  void rejectByStudent_ShouldThrow_WhenStatusInvalid() {
    ...
    assertThrows(
        IllegalStateException.class,
        () -> candidatureService.rejectByStudent(c.getId(), testStudent.getId()));
  }
  ...
  void rejectByEmployeur_ShouldThrow_WhenAlreadyAccepted() {
    ...
    assertThrows(
        IllegalStateException.class, () -> candidatureService.rejectByEmployeur(c.getId()));
  }
  ...
  void getCandidaturesByOffer_ShouldReturnApplications_WhenExists() {
    CandidatureDto c =
        candidatureService.postuler(testStudent.getId(), testOffer.getId(), testCv.getId());
    List<CandidatureEmployeurDto> list =
  ...
  void getCandidaturesByEmployeur_ShouldReturnApplications_WhenExists() {
    CandidatureDto c =
        candidatureService.postuler(testStudent.getId(), testOffer.getId(), testCv.getId());
    List<CandidatureEmployeurDto> list =
        candidatureService.getCandidaturesByEmployeur(testOffer.getEmployeurId());
