...
class InternshipOfferServiceTest {
  ...
  @InjectMocks private InternshipOfferService internshipOfferService;
  ...
  void setUp() {
    ...
    try {
      if (Files.exists(baseDir)) {
        Files.walk(baseDir).map(Path::toFile).forEach(java.io.File::delete);
      }
  ...
  void createOffer_validPdf_savesOffer() throws Exception {
    ...
    MockMultipartFile pdfFile =
        new MockMultipartFile("file", "offer.pdf", "application/pdf", pdfBytes);

    when(employeurRepository.findById(employeurDto.getId())).thenReturn(Optional.of(employeur));
  ...
  void createOffer_invalidPdf_throwsException() {
    ...
    MockMultipartFile invalidPdf =
        new MockMultipartFile("file", "bad.txt", "text/plain", "hello".getBytes());
    when(employeurRepository.findById(employeurDto.getId())).thenReturn(Optional.of(employeur));
  ...
  void getOfferPdf_readsBytesFromFile() throws Exception {
    ...
    InternshipOffer offer =
        InternshipOffer.builder().id(20L).employeur(employeur).pdfPath(filePath.toString()).build();
  ...
  void getOffersByEmployeurId_returnsList() {
    InternshipOffer offer1 =
        InternshipOffer.builder().id(1L).description("Stage A").employeur(employeur).build();
    InternshipOffer offer2 =
        InternshipOffer.builder().id(2L).description("Stage B").employeur(employeur).build();

    when(internshipOfferRepository.findOffersByEmployeurId(1L)).thenReturn(List.of(offer1, offer2));
