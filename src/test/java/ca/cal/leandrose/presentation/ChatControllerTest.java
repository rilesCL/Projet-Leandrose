...
class ChatControllerTest {
  ...
  void exchange_ShouldMaintainConversationContext() throws Exception {
    ...
    when(chatService.chat(eq(firstQuestion), eq(testSessionId))).thenReturn(firstResponse);
    when(chatService.chat(eq(secondQuestion), eq(testSessionId))).thenReturn(secondResponse);
  ...
  void clearChatSession_ShouldReturnNoContent_WhenSessionExists() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(delete("/gestionnaire/chatclient/session/{sessionId}", sessionId))
  ...
  void getActiveSessions_ShouldReturnListOfSessions() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/gestionnaire/chatclient/sessions"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
