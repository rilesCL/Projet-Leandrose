...
public class PDFGeneratorService {
  ...
  private void addPartiesInfo(Document document, EntenteStage entente) throws DocumentException {
    ...
    document.add(
        new Paragraph("L'ÉTUDIANT :", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10)));
    document.add(
    ...
    document.add(Chunk.NEWLINE);
    document.add(
        new Paragraph("L'ENTREPRISE :", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10)));
    ...
    document.add(Chunk.NEWLINE);
    document.add(
        new Paragraph("LE COLLÈGE :", FontFactory.getFont(FontFactory.HELVETICA_BOLD, 10)));
  ...
  private void addStageDetails(Document document, EntenteStage entente) throws DocumentException {
    ...
    addTableRow(
        ...
        cellFont);
    addTableRow(
        table, "Durée", entente.getDurationInWeeks() + " semaine(s)", cellBoldFont, cellFont);
  ...
  public String genererEvaluationPdf(
    ...
    try {
      ...
      String filename =
          "evaluation_" + evaluationStagiaire.getId() + "_" + UUID.randomUUID() + PDF_EXTENSION;
      ...
      log.error(
          "Erreur lors de la génération du PDF d'évaluation {}", evaluationStagiaire.getId(), e);
  ...
  private void addRatingLegendAligned(Document document, String language) throws DocumentException {
    ...
    Paragraph legendTitle =
        new Paragraph(
            "en".equals(language)
                ? "Rating scale (columns correspond to options below)"
                : "Échelle d'évaluation (colonnes correspondant aux cases)",
            legendTitleFont);
  ...
  private Map<String, List<Object>> getCategoriesMap(EvaluationFormData formData) {
    ...
    if (categoriesObj instanceof Map<?, ?> raw) {
      try {
  ...
  private Boolean getDiscussedWithTrainee(EvaluationFormData formData) {
    Object val =
        invokeGetter(
            formData,
            "discussedWithTrainee",
            "getDiscussedWithTrainee",
            "discussed",
            "isDiscussedWithTrainee");
  ...
  private Boolean getTechnicalTrainingSufficient(EvaluationFormData formData) {
    Object val =
        invokeGetter(
            formData,
            "technicalTrainingSufficient",
            "getTechnicalTrainingSufficient",
            "technicalTraining",
            "isTechnicalTrainingSufficient");
    if (val == null) return null;
    if (val instanceof Boolean) return (Boolean) val;
    String s = String.valueOf(val);
    if ("true".equalsIgnoreCase(s) || "yes".equalsIgnoreCase(s) || "1".equals(s)) return true;
  ...
  private void addEvaluationContent(Document document, EvaluationFormData formData, String language)
      throws DocumentException {
  ...
  private void addTraineeEvaluationPage(
      Document document,
      EvaluationFormData formData,
      String language,
      String profFirstName,
    ...
    document.add(
        new Paragraph(
            "en".equals(language) ? "Overall Assessment:" : "Appréciation globale :", normalFont));
    document.add(Chunk.NEWLINE);
    ...
    Paragraph appreciationTitle =
        new Paragraph(
            "en".equals(language) ? "SPECIFY YOUR ASSESSMENT:" : "PRÉCISEZ VOTRE APPRÉCIATION:",
            sectionFont);
    ...
    if (globalApp != null && !globalApp.isBlank()) {
      ...
      Paragraph emptySpace =
          new Paragraph("__________________________________________", normalFont);
    ...
    PdfPCell yesCell = new PdfPCell(pYes);
    yesCell.setBorder(Rectangle.NO_BORDER);
    PdfPCell noCell = new PdfPCell(pNo);
    noCell.setBorder(Rectangle.NO_BORDER);
    discussionTable.addCell(yesCell);
    discussionTable.addCell(noCell);
    ...
    Phrase py = new Phrase();
    if (yesW != null) py.add(new Chunk(yesW, 0, -2, true));
    else py.add(new Chunk(" "));
    py.add(new Chunk(" " + ("en".equals(language) ? "Yes" : "Oui"), normalFont));
    Phrase pn = new Phrase();
    if (noW != null) pn.add(new Chunk(noW, 0, -2, true));
    else pn.add(new Chunk(" "));
    pn.add(new Chunk(" " + ("en".equals(language) ? "No" : "Non"), normalFont));
    Phrase pm = new Phrase();
    if (maybeW != null) pm.add(new Chunk(maybeW, 0, -2, true));
    else pm.add(new Chunk(" "));
    ...
    PdfPCell cw1 = new PdfPCell(py);
    cw1.setBorder(Rectangle.NO_BORDER);
    PdfPCell cw2 = new PdfPCell(pn);
    cw2.setBorder(Rectangle.NO_BORDER);
    PdfPCell cw3 = new PdfPCell(pm);
    cw3.setBorder(Rectangle.NO_BORDER);
    welcomeTable.addCell(cw1);
    welcomeTable.addCell(cw2);
    welcomeTable.addCell(cw3);
    ...
    Phrase pYesT = new Phrase();
    if (yesT != null) pYesT.add(new Chunk(yesT, 0, -2, true));
    else pYesT.add(new Chunk("[ ]"));
    pYesT.add(new Chunk(" " + ("en".equals(language) ? "Yes" : "Oui"), normalFont));
    Phrase pNoT = new Phrase();
    if (noT != null) pNoT.add(new Chunk(noT, 0, -2, true));
    else pNoT.add(new Chunk("[ ]"));
    ...
    PdfPCell ct1 = new PdfPCell(pYesT);
    ct1.setBorder(Rectangle.NO_BORDER);
    PdfPCell ct2 = new PdfPCell(pNoT);
    ct2.setBorder(Rectangle.NO_BORDER);
    trainingTable.addCell(ct1);
    trainingTable.addCell(ct2);
    document.add(trainingTable);
    document.add(Chunk.NEWLINE);

    Paragraph name =
        new Paragraph(
            ("en".equals(language) ? "Name: " : "Nom : ")
                + "____________________    "
                + ("en".equals(language) ? "Function: " : "Fonction : ")
                + "____________________",
            normalFont);
    ...
    Paragraph signature =
        new Paragraph(
            ("en".equals(language) ? "Signature: " : "Signature : ")
                + "____________________    "
                + ("en".equals(language) ? "Date: " : "Date : ")
                + LocalDate.now().format(DATE_FORMATTER),
            normalFont);
    signature.setSpacingAfter(6f);
    document.add(signature);

    Paragraph returnTitle =
        new Paragraph(
            "en".equals(language)
  ...
  private String getRatingDisplayText(String rating, String language) {
    if ("en".equals(language)) {
      ...
      return switch (rating) {
        ...
        case "A_AMELIORER" ->
            "Totalement en désaccord / Les habiletés ne répondent pas aux attentes";
        default -> "N/A";
      };
    }
  }
  ...
  private void addHeaderEvaluation(Document document, String language, String name_college)
      throws DocumentException {
    Font headerFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 9, BaseColor.GRAY);
    String headerText =
        "en".equals(language)
            ? name_college + " - Work-Study Program"
            : name_college + " - Alternance travail-études";
  ...
  private void addTitleEvaluation(Document document, String language) throws DocumentException {
    Font titleFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 14, BaseColor.BLACK);
    String titleText =
        "en".equals(language) ? "Intern Evaluation Form" : "Fiche d'évaluation du stagiaire";
  ...
  private void addStudentAndCompanyTitle(
      Document document, EvaluationStagiaire evaluation, String language) throws DocumentException {
    Font normalFont = FontFactory.getFont(FontFactory.HELVETICA, 9, BaseColor.BLACK);
    ...
    addInfoRow(
        infoTable,
        getTranslation("program", language),
        translateProgram(evaluation.getStudent().getProgram(), language),
        boldFont,
        normalFont);
    addInfoRow(
        infoTable,
        getTranslation("company", language),
        evaluation.getInternshipOffer().getCompanyName(),
        boldFont,
        normalFont);
    addInfoRow(
        infoTable,
        getTranslation("supervisor", language),
        evaluation.getEmployeur().getFirstName() + " " + evaluation.getEmployeur().getLastName(),
        boldFont,
        normalFont);
    addInfoRow(
        infoTable,
        getTranslation("position", language),
        evaluation.getEmployeur().getCompanyName(),
        boldFont,
        normalFont);
    addInfoRow(
        infoTable,
        getTranslation("evaluationDate", language),
        evaluation.getDateEvaluation().format(DATE_FORMATTER),
        boldFont,
        normalFont);
    document.add(infoTable);
  }

  private void addGeneralComments(Document document, EvaluationFormData formData, String language)
      throws DocumentException {
    Object general =
        invokeGetter(
            formData,
    ...
    if (general != null) {
      ...
      if (!commentText.isBlank()) {
        ...
        String commentaireText =
            "en".equals(language) ? "General Comments" : "Commentaire généreaux";
    ...
    Map<String, String> programTranslations =
        Map.ofEntries(
            Map.entry(
                ...
                "en".equals(language) ? "Computer Science" : "Informatique"),
            Map.entry(
                Program.SOFTWARE_ENGINEERING.getTranslationKey(),
                "en".equals(language) ? "Software Engineering" : "Génie logiciel"),
            Map.entry(
                Program.INFORMATION_TECHNOLOGY.getTranslationKey(),
                "en".equals(language) ? "Information Technology" : "Technologie de l'information"),
            Map.entry(
                Program.DATA_SCIENCE.getTranslationKey(),
                "en".equals(language) ? "Data Science" : "Science des données"),
            Map.entry(
                Program.CYBER_SECURITY.getTranslationKey(),
                "en".equals(language) ? "Cyber Security" : "Cybersécurité"),
            Map.entry(
                Program.ARTIFICIAL_INTELLIGENCE.getTranslationKey(),
                "en".equals(language) ? "Artificial Intelligence" : "Intelligence artificielle"),
            Map.entry(
                Program.ELECTRICAL_ENGINEERING.getTranslationKey(),
                "en".equals(language) ? "Electrical Engineering" : "Génie électrique"),
            Map.entry(
                Program.MECHANICAL_ENGINEERING.getTranslationKey(),
                "en".equals(language) ? "Mechanical Engineering" : "Génie mécanique"),
            Map.entry(
                Program.CIVIL_ENGINEERING.getTranslationKey(),
                "en".equals(language) ? "Civil Engineering" : "Génie civil"),
            Map.entry(
                Program.CHEMICAL_ENGINEERING.getTranslationKey(),
                "en".equals(language) ? "Chemical Engineering" : "Génie chimique"),
            Map.entry(
                Program.BIOMEDICAL_ENGINEERING.getTranslationKey(),
                "en".equals(language) ? "Biomedical Engineering" : "Génie biomédical"),
            Map.entry(
                Program.BUSINESS_ADMINISTRATION.getTranslationKey(),
                "en".equals(language) ? "Business Administration" : "Administration des affaires"),
            Map.entry(
                Program.ACCOUNTING.getTranslationKey(),
                "en".equals(language) ? "Accounting" : "Comptabilité"),
            Map.entry(Program.FINANCE.getTranslationKey(), "Finance"),
            Map.entry(
                Program.ECONOMICS.getTranslationKey(),
                "en".equals(language) ? "Economics" : "Économie"),
            Map.entry(Program.MARKETING.getTranslationKey(), "Marketing"),
            Map.entry(
                Program.MANAGEMENT.getTranslationKey(),
                "en".equals(language) ? "Management" : "Gestion"),
            Map.entry(
                Program.PSYCHOLOGY.getTranslationKey(),
                "en".equals(language) ? "Psychology" : "Psychologie"),
            Map.entry(
                Program.SOCIOLOGY.getTranslationKey(),
                "en".equals(language) ? "Sociology" : "Sociologie"),
            Map.entry(
                Program.POLITICAL_SCIENCE.getTranslationKey(),
                "en".equals(language) ? "Political Science" : "Science politique"),
            Map.entry(
                Program.INTERNATIONAL_RELATIONS.getTranslationKey(),
                "en".equals(language) ? "International Relations" : "Relations internationales"),
            Map.entry(Program.LAW.getTranslationKey(), "en".equals(language) ? "Law" : "Droit"),
            Map.entry(
                Program.EDUCATION.getTranslationKey(),
                "en".equals(language) ? "Education" : "Éducation"),
            Map.entry(
                Program.LITERATURE.getTranslationKey(),
                "en".equals(language) ? "Literature" : "Littérature"),
            Map.entry(
                Program.HISTORY.getTranslationKey(),
                "en".equals(language) ? "History" : "Histoire"),
            Map.entry(
                Program.PHILOSOPHY.getTranslationKey(),
                "en".equals(language) ? "Philosophy" : "Philosophie"),
            Map.entry(
                Program.LINGUISTICS.getTranslationKey(),
                "en".equals(language) ? "Linguistics" : "Linguistique"),
            Map.entry(
                Program.BIOLOGY.getTranslationKey(),
                "en".equals(language) ? "Biology" : "Biologie"),
            Map.entry(
                Program.CHEMISTRY.getTranslationKey(),
                "en".equals(language) ? "Chemistry" : "Chimie"),
            Map.entry(
                Program.PHYSICS.getTranslationKey(),
                "en".equals(language) ? "Physics" : "Physique"),
            Map.entry(
                Program.MATHEMATICS.getTranslationKey(),
                "en".equals(language) ? "Mathematics" : "Mathématiques"),
            Map.entry(
                Program.STATISTICS.getTranslationKey(),
                "en".equals(language) ? "Statistics" : "Statistiques"),
            Map.entry(
                Program.ENVIRONMENTAL_SCIENCE.getTranslationKey(),
                "en".equals(language) ? "Environmental Science" : "Science environnementale"),
            Map.entry(
                Program.MEDICINE.getTranslationKey(),
                "en".equals(language) ? "Medicine" : "Médecine"),
            Map.entry(
                Program.NURSING.getTranslationKey(),
                "en".equals(language) ? "Nursing" : "Soins infirmiers"),
            Map.entry(
                Program.PHARMACY.getTranslationKey(),
                "en".equals(language) ? "Pharmacy" : "Pharmacie"),
            Map.entry(
                Program.DENTISTRY.getTranslationKey(),
                "en".equals(language) ? "Dentistry" : "Dentisterie"),
            Map.entry(Program.ARCHITECTURE.getTranslationKey(), "Architecture"),
            Map.entry(
                Program.FINE_ARTS.getTranslationKey(),
                "en".equals(language) ? "Fine Arts" : "Beaux-arts"),
            Map.entry(
                Program.MUSIC.getTranslationKey(), "en".equals(language) ? "Music" : "Musique"),
            Map.entry(
                Program.THEATER.getTranslationKey(), "en".equals(language) ? "Theater" : "Théâtre"),
            Map.entry(
                Program.FILM_STUDIES.getTranslationKey(),
                "en".equals(language) ? "Film Studies" : "Études cinématographiques"),
            Map.entry(Program.COMMUNICATION.getTranslationKey(), "Communication"),
            Map.entry(
                Program.JOURNALISM.getTranslationKey(),
                "en".equals(language) ? "Journalism" : "Journalisme"),
            Map.entry(Program.DESIGN.getTranslationKey(), "Design"),
            Map.entry(
                Program.ANTHROPOLOGY.getTranslationKey(),
                "en".equals(language) ? "Anthropology" : "Anthropologie"),
            Map.entry(
                Program.GEOGRAPHY.getTranslationKey(),
                "en".equals(language) ? "Geography" : "Géographie"),
            Map.entry(
                Program.SPORTS_SCIENCE.getTranslationKey(),
                "en".equals(language) ? "Sports Science" : "Science du sport"));
